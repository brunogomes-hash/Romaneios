...
for pasta in pastas_busca:
    if os.path.exists(pasta):
        for f in os.listdir(pasta):
            # Garante que pega apenas as imagens originais e ignora as já assinadas
...
if not arquivos_encontrados:
    ...
    if caminho_final_imagem:
        ...
        try:
            ...
            # Ajusta o tamanho para a tela do celular de forma proporcional
            largura_display = 600
            ...
            if st.button("💾 Enviar Romaneio Assinado"):
                if canvas_result.image_data is not None and np.any(canvas_result.image_data[:, :, 3] > 0):
                    with st.spinner("🔄 Gravando assinatura permanentemente no documento..."):
